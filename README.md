...
歡迎來到國立臺灣科技大學機器人研究社 (NTUST Robot Researchers Club) 的社團管理系統專案。
本系統旨在協助社團管理器材借用、財務報帳、機臺設備借用、社團行事曆排程、社課資源分享、競賽意願調查等日常行政事務，並提供對外宣傳平台。
...
| 表格處理  | ExcelJS + SheetJS (xlsx)                  |
| 行事曆協定| iCalendar (RFC 5545 / RFC 7986) 串流      |
| 通知提示  | Sonner                                    |
| 日期處理  | date-fns                                  |
| 輪播組件  | Embla Carousel                            |
...
└── modules/
    ├── admin/         # 後台管理
    ├── announcement/  # 公告發布與多軌推播
    ├── auth/          # 認證 (登入/註冊/學校信箱 OTP 密碼重設)
    ├── calendar/      # 行事曆活動日程、Excel 批次排程與 ICS 同步
    ├── course/        # 課程管理、講義直連下載與開課推播
    ├── equipment/     # 器材借用、審核與盤點總表
    ├── finance/       # 財務報帳與核銷撥款
    ├── machine/       # 機臺設備借用 (3D列印/雷切) 與排程防撞避碰
...
### 社團行事曆與即時同步訂閱 (Calendar & iCal Sync)

- **公開行事曆（`/calendar`）**：彙整全學年社課、社務會議、競賽與創客活動，提供月份檢視 (Month) 與列表檢視 (List)。
- **跨平台 iCal / ICS 串流即時訂閱**：提供符合 RFC 5545 / RFC 7986 規範之訂閱端點（`/api/calendar/ics`），支援 Google 日曆、Apple 行事曆 (iOS/macOS) 與 Outlook 自動背景排程同步更新。
- **公版行事曆 PDF 匯出列印**：精準支援 A4 橫式列印排版，一鍵由瀏覽器列印為官方公版日程 PDF。
- **後台 Excel 批次排程**：幹部可直接上傳 Excel 檔案批次解析匯入整學期行程，並支援單筆活動即時增刪改。

### 社團課程專區 (Courses)

- 學期分類篩選（例如 `113-2`、`114-1`）與大綱簡介
- 公開 / 社員限定教材權限區分
- **單步驟直連極速下載**：消除舊版兩階段 Token 交換的 400 過期快取錯誤與冷啟動延遲，直接由 Next.js BFF 自動附帶 Session 權限校驗串流回傳 Google Drive 私有教材
- 線上課堂錄影（串流回放）與教材投影片即時查閱
...
- 跨系所組隊媒合與經費指導意願調查（`/dashboard/competitions`，籌備中）
...
- 器材總表維護、借用審核、實體庫存盤點與點收
- 機臺借用排程審核、衝突時段檢視與時段管理
- 行事曆日程管理與 Excel 批次匯入
- 財務單據線上初審、實體發票點收與撥款標記
- 人員驗證碼派發、身份組變更與公告課程管理

### 伺服器保活與防冷啟動機制 (Keep-Warm & Health Check)

- 結合 GitHub Actions 工作流（每 10 分鐘自動排程探活）與 GAS 時間驅動觸發器，常態維持 Serverless 容器與 SheetDB 熱機狀態，徹底消除冷啟動延遲。
